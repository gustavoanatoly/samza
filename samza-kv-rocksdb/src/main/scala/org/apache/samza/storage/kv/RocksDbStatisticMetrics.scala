...
class RocksDbStatisticMetrics(val storeName: String = "unknown",
  ...
  def blockCacheMiss() = counterBlockCacheMiss
  def blockCacheHit() = counterBlockCacheHit
  def blockCacheAdd() = counterBlockCacheAdd
  def blockCacheIndexMiss() = counterBlockCacheIndexMiss
  def blockCacheIndexHit() = counterBlockCacheIndexHit
  def blockCacheFilterMiss() = counterBlockCacheFilterMiss
  def blockCacheFilterHit() = counterBlockCacheFilterHit
  def blockCacheDataMiss() = counterBlockCacheDataMiss
  def blockCacheDataHit() = counterBlockCacheDataHit
  def bloomFilterUseful() = counterBloomFilterUseful
  def memtableHit() = counterMemtableHit
  def memtableMiss() = counterMemtableMiss
  def getHitL0() = counterGetHitL0
  def getHitL1() = counterGetHitL1
  def getHitL2AndUp() = counterGetHitL2AndUp
  def compactionKeyDropNewerEntry() = counterCompactionKeyDropNewerEntry
  def compactionKeyDropObsolete() = counterCompactionKeyDropObsolete
  def compactionKeyDropUser() = counterCompactionKeyDropUser
  def numberKeysWritten() = counterNumberKeysWritten
  def numberKeysRead() = counterNumberKeysRead
  def numberKeysUpdated() = counterNumberKeysUpdated
  def bytesWritten() =  counterBytesWritten
  def bytesRead() = counterBytesRead
  def noFileCloses() = counterNoFileCloses
  def noFileOpens() = counterNoFileOpens
  def noFileErrors() = counterNoFileErrors
  def stallL0SlowdownMicros() = counterStallL0SlowdownMicros
  def stallMemtableCompactionMicros() = counterStallMemtableCompactionMicros
  def stallL0NumFilesMicros() = counterStallL0NumFilesMicros
  def stallMicros() = counterStallMicros
  def dbMutexWaitMicros() = counterDbMutexWaitMicros
  def rateLimitDelayMillis() = counterRateLimitDelayMillis
  def noIterators() = counterNoIterators
  def numberMultigetCalls() = counterNumberMultigetCalls
  def numberMultigetKeysRead() = counterNumberMultigetKeysRead
  def numberMultigetBytesRead() = counterNumberMultigetBytesRead
  def numberFilteredDeletes() = counterNumberFilteredDeletes
  def numberMergeFailures() = counterNumberMergeFailures
  def sequenceNumber() = counterSequenceNumber
  def bloomFilterPrefixChecked() = counterBloomFilterPrefixChecked
  def bloomFilterPrefixUseful() = counterBloomFilterPrefixUseful
  def numberOfReseeksInIteration() = counterNumberOfReseeksInIteration
  def getUpdatesSinceCalls() = counterGetUpdatesSinceCalls
  def blockCacheCompressedMiss() = counterBlockCacheCompressedMiss
  def blockCacheCompressedHit() = counterBlockCacheCompressedHit
  def walFileSynced() = counterWalFileSynced
  def walFileBytes() = counterWalFileBytes
  def walDoneBySelf() = counterWriteDoneBySelf
  def writeDoneByOther() = counterWriteDoneByOther
  def writeTimeout() = counterWriteTimedOut
  def writeWithWal() = counterWriteWithWall
  def compactReadBytes() = counterCompactReadBytes
  def compactWriteBytes() = counterCompactWriteBytes
  def flushWriteBytes() = counterFlushWriteBytes
  def numberDirectLoadTableProperties() = counterNumberDirectLoadTableProperties
  def numberSupervisionAcquiries() = counterNumberSupervisionAcquires
  def numberSupervisionReleases() = counterNumberSupervisionReleases
  def numberSupervisionCleanups() = counterNumberSupervisionCleanups
  def numberBlockNotCompressed() = counterNumberBlockNoCompressed
  def compactionOutfileSyncMicrosHistogram() = gaugeCompactionOutfileSyncMicrosHistogram
  def compactionTimeHistogram() = gaugeCompactionTimeHistogram

  def dbGetHistogram() = gaugeDbGetHistogram
  def dbMultiGetHistogram() = gaugeDbMultiGetHistogram
  def dbSeekHistogram() = gaugeDbSeekHistogram
  def dbWriteHistogram() = gaugeDbWriteHistogram
  def hardRateLimitDelayCountHistogram() = gaugeHardRateLimitDelayCountHistogram
  def manifestFileSyncMicrosHistogram() = gaugeManifestFileSyncMicrosHistogram
  def numFilesInSingleCompactionHistogram() = gaugeNumFilesInSingleCompactionHistogram
  def readBlockCompactionMicrosHistogram() = gaugeReadBlockCompactionMicrosHistogram
  def readBlockGetMicrosHistogram() = gaugeReadBlockGetMicrosHistogram
  def softRateLimitDelayCountHistogram() = gaugeSoftRateLimitDelayCountHistogram
  def stallL0NumFilesCountHistogram() = gaugeStallL0NumFilesCountHistogram
  def stallL0SlowdownCountHistogram() = gaugeStallL0SlowdownCountHistogram
  def stallMemtableCompactionCountHistogram() = gaugeStallMemtableCompactionCountHistogram
  def tableOpenIOMicrosHistogram() = gaugeTableOpenIOMicrosHistogram
  def tableSyncMicrosHistogram() = gaugeTableSyncMicrosHistogram
  def walFileSyncMicrosHistogram() = gaugeWalFileSyncMicrosHistogram
  def writeRawBlockMicrosHistogram() = gaugeWriteRawBlockMicrosHistogram
  def writeStallHistogram() = gaugeWriteStallHistogram
}
